...
async function recognizeReviewItem(item) {
  try {
    ...
    const imageForOcr = await prepareImageForOcr(item.file);
    const result = await window.Tesseract.recognize(imageForOcr, "chi_sim+eng", {
...
async function prepareImageForOcr(file) {
  try {
    ...
    const scale = Math.min(Math.max(1400 / Math.min(bitmap.width, bitmap.height), 1), 1.7);
...
async function openSource(account) {
  ...
  const launchUrl = account.directAppUrl || account.shortcutUrl || account.appUrl;
...
function openAccountDialog(account = null) {
  ...
  accountForm.elements.directAppUrl.value = account?.directAppUrl ?? "";
  accountForm.elements.shortcutUrl.value = account?.shortcutUrl ?? "";
...
function applyDefaultUrls(type) {
  ...
  if (!accountForm.elements.directAppUrl.value) {
    accountForm.elements.directAppUrl.value = defaultDirectAppUrls[type] ?? "";
  }
  if (!accountForm.elements.shortcutUrl.value && shortcutName) {
    accountForm.elements.shortcutUrl.value = createShortcutUrl(shortcutName);
  }
}
...
function normalizeAccount(account) {
  ...
  const migratedUrl = typeof account?.appUrl === "string" ? account.appUrl.trim() : "";
  const directFallback = migratedUrl && !isShortcutUrl(migratedUrl) ? migratedUrl : defaultDirectAppUrls[type] ?? "";
  const shortcutFallback = isShortcutUrl(migratedUrl) ? migratedUrl : createDefaultShortcutUrl(account, type);
  const directAppUrl = normalizeAppUrl(account?.directAppUrl, directFallback);
  const shortcutUrl = normalizeAppUrl(account?.shortcutUrl, shortcutFallback);
  return {
    ...
    directAppUrl,
    shortcutUrl,
    appUrl: directAppUrl || shortcutUrl,
    webUrl: normalizeWebUrl(account?.webUrl ?? account?.loginUrl, ""),
    enabled: account?.enabled !== false
  };
}

function createDefaultShortcutUrl(account, type) {
...
function isShortcutUrl(value) {
  return typeof value === "string" && value.trim().toLowerCase().startsWith("shortcuts:");
}
...
function parseAmountsFromText(text) {
  const matches = normalizeOcrText(text).match(amountPattern()) ?? [];
  return [
    ...new Set(
      matches
        .map(parseOcrAmount)
...
function extractPreferredAmounts(text) {
  const source = normalizeKeywordText(text).replace(/\s+/g, "");
  const keywords = ["总资产", "总资严", "总资广", "总资声", "资产总额", "总金额", "总金颤", "账户总资产", "账户资产", "资产合计", "合计资产", "总市值", "总余额", "存款总额"];
  const amounts = [];
  for (const keyword of keywords) {
    let start = source.indexOf(keyword);
    while (start !== -1) {
      const match = source.slice(start + keyword.length, start + keyword.length + 120).match(amountPattern())?.[0];
      if (match) {
        const amount = parseOcrAmount(match);
...
function amountPattern() {
  return /(?:¥|￥|RMB|CNY)?\s*(?:[0-9ＯｏOＩｌlＳs]{1,3}(?:,[0-9ＯｏOＩｌlＳs]{3})+|[0-9ＯｏOＩｌlＳs]+)(?:\.[0-9ＯｏOＩｌlＳs]{1,2})?/gi;
}
